...
pub async fn ensure_admin_server(
    ...
    let (axum_server, server_handle) = match crate::proxy::AxumServer::start(
        ...
        integration.clone(),
        cloudflared_state,
        { #[cfg(not(target_os = "android"))] { config.proxy_pool.clone() } #[cfg(target_os = "android")] { Default::default() } },
    )
    .await
    {
...
pub async fn update_model_mapping(
    ...
    #[cfg(not(target_os = "android"))]
    #[cfg(not(target_os = "android"))] app_config.proxy.custom_mapping = config.custom_mapping;
...
pub async fn set_preferred_account(
    ...
    if let Some(instance) = instance_lock.as_ref() {
        ...
        #[cfg(not(target_os = "android"))]
    #[cfg(not(target_os = "android"))] app_config.proxy.preferred_account_id = cleaned_id.clone();
