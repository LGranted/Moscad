...
#[cfg(not(desktop))]
#[tauri::command]
pub async fn toggle_auto_launch(_app: tauri::AppHandle, _enable: bool) -> Result<(), String> {
    Ok(())
}

#[cfg(not(desktop))]
#[tauri::command]
pub async fn is_auto_launch_enabled(_app: tauri::AppHandle) -> Result<bool, String> {
    Ok(false)
}
// Autostart 命令
