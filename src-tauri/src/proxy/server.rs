...
async fn admin_save_config(
    ...
    // 更新安全策略
    {
        ...
        *security = crate::proxy::ProxySecurityConfig::from_proxy_config(#[cfg(not(target_os = "android"))] &new_config.proxy #[cfg(target_os = "android")] &Default::default());
    }
...
async fn admin_start_proxy_service(State(state): State<AppState>) -> impl IntoResponse {
    ...
    if let Ok(mut config) = crate::modules::config::load_app_config() {
        #[cfg(not(target_os = "android"))] config.proxy.auto_start = true;
...
async fn admin_stop_proxy_service(State(state): State<AppState>) -> impl IntoResponse {
    // 1. 持久化配置 (修复 #1166)
    if let Ok(mut config) = crate::modules::config::load_app_config() {
        #[cfg(not(target_os = "android"))] config.proxy.auto_start = false;
        let _ = crate::modules::config::save_app_config(&config);
    }
...
async fn admin_update_model_mapping(
    ...
    #[cfg(not(target_os = "android"))] app_config.proxy #[cfg(target_os = "android")] Default::default().custom_mapping = config.custom_mapping;
...
async fn admin_get_security_config(
    ...
    Ok(Json(#[cfg(not(target_os = "android"))] app_config.proxy #[cfg(target_os = "android")] Default::default().security_monitor))
}
...
async fn admin_update_security_config(
    ...
    #[cfg(not(target_os = "android"))] app_config.proxy #[cfg(target_os = "android")] Default::default().security_monitor = config.clone();
...

    {
        ...
        *sec = crate::proxy::ProxySecurityConfig::from_proxy_config(&#[cfg(not(target_os = "android"))] app_config.proxy #[cfg(target_os = "android")] Default::default());
