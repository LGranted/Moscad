...
use dashmap::DashMap;
#[cfg(not(target_os = "android"))] use rquest::Client; #[cfg(target_os = "android")] use reqwest::Client;
use futures::{stream, StreamExt};
use std::time::Duration;
use crate::proxy::config::{ProxyPoolConfig, ProxySelectionStrategy, ProxyEntry};

#[cfg(not(target_os = "android"))] use rquest_util::Emulation;
...
impl ProxyPoolManager {
    ...
    async fn persist_bindings(&self) {
        ...
        if let Ok(mut app_config) = crate::modules::config::load_app_config() {
            ...
            #[cfg(not(target_os = "android"))]
            #[cfg(not(target_os = "android"))] { app_config.proxy.proxy_pool = config.clone(); }
