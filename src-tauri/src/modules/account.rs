...
pub fn delete_account(account_id: &str) -> Result<(), String> {
    ...
    #[cfg(not(target_os = "android"))] { crate::proxy::server::trigger_account_delete(account_id); }
...
pub async fn switch_account(
    ...
    if account.device_profile.is_none() {
        ...
        let new_profile: crate::models::DeviceProfile = Default::default();
...
pub fn update_account_quota(account_id: &str, quota: QuotaData) -> Result<(), String> {
        ...
        if config.quota_protection.enabled {
            if let Some(ref q) = account.quota {
                ...
                for model in &q.models {
                    if let Some(std_id) =
                        Some({ let _ = &model.name; model.name.clone() }) // android stub
                    {
    ...
    #[cfg(not(target_os = "android"))] { crate::proxy::server::trigger_account_reload(account_id); }
    ...
    let token = match oauth::ensure_fresh_token(&account.token, Some(&account.id)).await {
        ...
        Err(e) => {
            if e.contains("invalid_grant") {
                ...
                #[cfg(not(target_os = "android"))] { crate::proxy::server::trigger_account_reload(&account.id); }
            }
...
pub async fn check_and_trigger_warmup_for_recovered_models() {
    ...
    for account in accounts {
        ...
        #[cfg(not(target_os = "android"))]
        #[cfg(not(target_os = "android"))] crate::modules::scheduler::trigger_warmup_for_account(&account).await;
    }
}
