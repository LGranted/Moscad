...
pub async fn warmup_model_directly(
    ...
    let port = config::load_app_config()
        .map(|_c| { #[cfg(not(target_os = "android"))] { _c.proxy.port } #[cfg(target_os = "android")] { 8045u16 } })
        .unwrap_or(8045);
    ...
    loop {
        ...
        if !warmup_items.is_empty() {
            ...
            warmup_items.retain(|(_, email, model, _, _, _)| {
                let _history_key = format!("{}:{}:100", email, model);
                #[cfg(not(target_os = "android"))]
                { !crate::modules::scheduler::check_cooldown(&_history_key, 14400) }
                #[cfg(target_os = "android")]
                { true }
            });

            let total_after = warmup_items.len();
            if total_after < total_before {
                info!("Filtered out {} already warmed up models", total_before - total_after);
            }
                    ...
                    for handle in handles {
                        match handle.await {
                            Ok((true, email, model)) => {
                                ...
                                let _history_key = format!("{}:{}:100", email, model);
                                #[cfg(not(target_os = "android"))]
                                #[cfg(not(target_os = "android"))] crate::modules::scheduler::record_warmup_history(&_history_key, _now_ts);
                            }
                            _ => {}
                        }
                    }
    ...
    tokio::spawn(async move {
        for (name, pct) in models_to_warm {
            if warmup_model_directly(&token, &name, &pid, &email, pct, Some(&account_id_clone)).await {
                let _history_key = format!("{}:{}:100", email, name);
                let _now_ts = chrono::Utc::now().timestamp();
                #[cfg(not(target_os = "android"))]
                crate::modules::scheduler::record_warmup_history(&_history_key, _now_ts);
            }
