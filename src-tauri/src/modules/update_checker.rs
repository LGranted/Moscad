...
async fn create_client() -> Result<reqwest::Client, String> {
    ...
    if let Ok(config) = crate::modules::config::load_app_config() {
        if #[cfg(not(target_os = "android"))] config.proxy.upstream_proxy.enabled #[cfg(target_os = "android")] false && !config.proxy.upstream_proxy.url.is_empty() {
#[cfg(not(target_os = "android"))] #[cfg(not(target_os = "android"))]             logger::log_info(&format!("Update checker using upstream proxy: {}", config.proxy.upstream_proxy.url));
#[cfg(not(target_os = "android"))] #[cfg(not(target_os = "android"))]             match reqwest::Proxy::all(&config.proxy.upstream_proxy.url) {
#[cfg(not(target_os = "android"))] #[cfg(not(target_os = "android"))]                 Ok(proxy) => {
#[cfg(not(target_os = "android"))] #[cfg(not(target_os = "android"))]                     builder = builder.proxy(proxy);
#[cfg(not(target_os = "android"))] #[cfg(not(target_os = "android"))]                 },
#[cfg(not(target_os = "android"))] #[cfg(not(target_os = "android"))]                 Err(e) => {
#[cfg(not(target_os = "android"))] #[cfg(not(target_os = "android"))]                     logger::log_warn(&format!("Failed to parse proxy URL '{}': {}", config.proxy.upstream_proxy.url, e));
                }
            }
        }
    }
