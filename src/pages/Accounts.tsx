        ...
        <div className="px-4 pb-4 space-y-3 border-t border-gray-100 dark:border-gray-700 pt-3">
          {quota?.error ? (
            ...
            <div className="space-y-1">
              ...
              {quota.quotas.length > 5 && <p className="text-xs text-gray-400">+{quota.quotas.length - 5} ещё...</p>}
            </div>
          ) : (
...
export const Accounts: React.FC = () => {
  const { accounts, quotas, loading, fetchAccounts, deleteAccount, setCurrentAccount, clearRateLimit, fetchQuotaForAll } = useAccountStore();
  ...
  const handleModalClose = useCallback(() => { setShowAdd(false); refresh(); }, [refresh]);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-24">
      <header className="sticky top-0 z-10 bg-gray-50 dark:bg-gray-950 px-4 pt-safe pt-4 pb-3 flex items-center justify-between border-b border-gray-100 dark:border-gray-800">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-blue-600 text-white text-sm font-semibold active:bg-blue-700">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => saveConfig({ rotation_enabled: !config.rotation_enabled })}
              className={`relative w-12 h-6 rounded-full transition-colors ${config.rotation_enabled ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'}`}>
          ...
          {config.rotation_enabled && (
            <div className="flex gap-2">
              {(['round-robin', 'by-limit'] as const).map((s) => (
                <button key={s} onClick={() => saveConfig({ rotation_strategy: s })}
                  className={`flex-1 py-2 rounded-xl text-xs font-medium transition-colors ${config.rotation_strategy === s ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>
