  ...
  return (
    <div className="mx-4 rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${status.is_running ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <button onClick={() => copy(proxyUrl, 'url')} className="p-2 rounded-xl bg-gray-100 dark:bg-gray-700 active:opacity-60">
          <span className="text-xs">{copied === 'url' ? '✓' : '⎘'}</span>
        </button>
      </div>
      <div className="flex items-center gap-2 mb-4">
        <div className="flex-1 bg-gray-50 dark:bg-gray-700 rounded-xl px-3 py-2 flex items-center gap-2">
          <span className="text-xs text-gray-500 shrink-0">Ключ</span>
          <span className="text-sm font-mono text-gray-800 dark:text-gray-200">{apiKey}</span>
        </div>
        <button onClick={() => copy(apiKey, 'key')} className="p-2 rounded-xl bg-gray-100 dark:bg-gray-700 active:opacity-60">
          <span className="text-xs">{copied === 'key' ? '✓' : '⎘'}</span>
        </button>
      </div>
      <button onClick={status.is_running ? stop : start} disabled={loading}
        className={`w-full py-3 rounded-xl font-semibold text-sm transition-colors ${status.is_running ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 active:bg-red-200' : 'bg-blue-600 text-white active:bg-blue-700'} disabled:opacity-50`}>
        {loading ? 'Подождите...' : status.is_running ? 'Остановить прокси' : 'Запустить прокси'}
      </button>
      <button onClick={fetchStatus} className="w-full mt-2 py-2 text-xs text-gray-400 active:text-gray-600">Обновить статус</button>
    </div>
  );
};
...
const QuotaCard: React.FC<{ quota: AggregatedQuota }> = ({ quota }) => {
  const reqPct = Math.min(100, (quota.total_requests_remaining / 1000) * 100);
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4">
      <div className="flex items-start justify-between mb-3">
        <div>
          <p className="text-sm font-semibold text-gray-900 dark:text-white">{modelLabel(quota.model)}</p>
          <p className="text-xs text-gray-400">{quota.accounts_count} аккаунт(ов)</p>
        </div>
        <div className="text-right">
          <p className="text-lg font-bold text-blue-600 dark:text-blue-400">{formatNumber(quota.total_requests_remaining)}</p>
          <p className="text-xs text-gray-400">запросов</p>
        </div>
      </div>
      <div className="h-1.5 rounded-full bg-gray-100 dark:bg-gray-700">
        <div className="h-full rounded-full bg-blue-500 transition-all" style={{ width: `${reqPct}%` }} />
      </div>
      {quota.total_tokens_remaining > 0 && (
        <p className="text-xs text-gray-400 mt-1 text-right">{formatNumber(quota.total_tokens_remaining)} токенов</p>
      )}
    </div>
  );
};
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-24">
      <header className="sticky top-0 z-10 bg-gray-50 dark:bg-gray-950 px-4 pt-4 pb-3 flex items-center justify-between border-b border-gray-100 dark:border-gray-800">
        ...
        <button onClick={refresh} className="p-2 rounded-xl active:bg-gray-200 dark:active:bg-gray-700">
      ...
      <div className="space-y-4 pt-4">
        ...
        <div className="mx-4 flex gap-3">
          <div className="flex-1 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-3 text-center">
            ...
            <p className="text-xs text-gray-400">аккаунтов</p>
          </div>
          <div className="flex-1 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-3 text-center">
          ...
          <div className="flex-1 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-3 text-center">
            ...
            <p className="text-xs text-gray-400">моделей</p>
          </div>
        </div>
        <div className="mx-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-gray-900 dark:text-white">Лимиты по моделям</h2>
            <button onClick={refreshQuotas} disabled={quotaLoading} className="text-xs text-blue-600 dark:text-blue-400 disabled:opacity-40">
          ...
          {aggregated.length === 0 && !quotaLoading && (
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6 text-center">
              <p className="text-gray-400 text-sm">{accounts.length === 0 ? 'Добавьте аккаунты для просмотра лимитов' : 'Нажмите «Обновить» для загрузки квот'}</p>
            </div>
          )}
          {quotaLoading && aggregated.length === 0 && (
            <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="bg-white dark:bg-gray-800 rounded-2xl h-20 animate-pulse border border-gray-100 dark:border-gray-700" />)}</div>
          )}
          <div className="space-y-3">{aggregated.map(q => <QuotaCard key={q.model} quota={q} />)}</div>
        </div>
        {accounts.length > 0 && (
          <div className="mx-4">
            <button onClick={handleClearAll} disabled={clearingLimits}
              className="w-full py-3 rounded-2xl border border-orange-200 dark:border-orange-800 text-orange-600 dark:text-orange-400 text-sm font-medium active:bg-orange-50 disabled:opacity-50">
